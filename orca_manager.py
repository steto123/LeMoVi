...
def normalize_basis_alias(basis_str):
    """Normalizes basis set names for robust alias matching (e.g. 6-31G(d) and 6-31G*, 6-311G(d) and 6-311G*)."""
    if not basis_str:
        return ""
    b = str(basis_str).strip().upper()
    b = re.sub(r'\s*,\s*', ',', b)
    b = re.sub(r'\((\w+)\s+(\w+)\)', r'(\1,\2)', b)
    if b.endswith("**"):
        b = b[:-2] + "(D,P)"
    elif b.endswith("*"):
        b = b[:-1] + "(D)"
...
DEFAULT_TMS_REFERENCES = [
  ...
  {"geom_method": "HF", "geom_basis": "6-31G(d)", "method": "HF", "basis": "6-31G(d)", "h1_shielding": 32.9035, "c13_shielding": 201.7285, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-31+G(d)", "method": "HF", "basis": "6-31+G(d)", "h1_shielding": 32.819, "c13_shielding": 201.8459, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-31++G(d)", "method": "HF", "basis": "6-31++G(d)", "h1_shielding": 32.8185, "c13_shielding": 202.1032, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-31G(d,p)", "method": "HF", "basis": "6-31G(d,p)", "h1_shielding": 32.3358, "c13_shielding": 203.1555, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-31+G(d,p)", "method": "HF", "basis": "6-31+G(d,p)", "h1_shielding": 32.2849, "c13_shielding": 203.1713, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-31++G(d,p)", "method": "HF", "basis": "6-31++G(d,p)", "h1_shielding": 32.2918, "c13_shielding": 203.5056, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311G", "method": "HF", "basis": "6-311G", "h1_shielding": 33.6399, "c13_shielding": 203.5439, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311G(d)", "method": "HF", "basis": "6-311G(d)", "h1_shielding": 32.8505, "c13_shielding": 195.989, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311+G(d)", "method": "HF", "basis": "6-311+G(d)", "h1_shielding": 32.7994, "c13_shielding": 195.9349, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311++G(d)", "method": "HF", "basis": "6-311++G(d)", "h1_shielding": 32.7887, "c13_shielding": 196.15, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311G(d,p)", "method": "HF", "basis": "6-311G(d,p)", "h1_shielding": 32.4875, "c13_shielding": 196.2165, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311+G(d,p)", "method": "HF", "basis": "6-311+G(d,p)", "h1_shielding": 32.4645, "c13_shielding": 195.9187, "source": "[2]"},
  {"geom_method": "HF", "geom_basis": "6-311++G(d,p)", "method": "HF", "basis": "6-311++G(d,p)", "h1_shielding": 32.4549, "c13_shielding": 196.1819, "source": "[2]"},
  {"geom_method": "k.A.", "geom_basis": "k.A.", "method": "HF", "basis": "def2-TZVPP", "h1_shielding": None, "c13_shielding": 194.1, "source": "[4]"},
  {"geom_method": "B3LYP", "geom_basis": "6-31G(d)", "method": "B3LYP", "basis": "6-31G(d)", "h1_shielding": 32.5976, "c13_shielding": 199.9853, "source": "[2]"},
...
def find_best_tms_match(method, basis, geom_method=None, geom_basis=None, tms_list=None,
    ...
    basis_clean = normalize_basis_alias(basis)
    gm_clean = str(geom_method).strip().upper() if geom_method else None
    gb_clean = normalize_basis_alias(geom_basis) if geom_basis else None
    ...
    def match_nmr(e):
        return (normalize_method_alias(e.get("method")) == normalize_method_alias(method) and
                normalize_basis_alias(e.get("basis", "")) == basis_clean)
    ...
    def match_geom(e):
        return (normalize_method_alias(e.get("geom_method")) == normalize_method_alias(geom_method) and
                normalize_basis_alias(e.get("geom_basis", "")) == gb_clean)
...
class OrcaJobManager:
    ...
    def create_job(self, name, xyz_content, charge=0, multiplicity=1, task="Opt",
                   ...
                   use_draco=False, execution_mode="local", slurm_config=None):
        ...
        meta = {
            ...
            "slurm_config": slurm_config or {}
        }
        
        self._write_meta(job_dir, meta)
        return job_id

    def _generate_slurm_script(self, job_name, remote_job_dir, config):
        env_lines = config.get("environment", [
            "export MODULEPATH=/srv/software/eb24/modules/all:$MODULEPATH",
            "module load chem/ORCA/6.0.0-gompi-2023b",
            "module load mpi/OpenMPI",
            "export OMP_NUM_THREADS=$SLURM_CPUS_PER_TASK"
        ])
        env_block = "\n".join(env_lines)
        return f"""#!/bin/bash -l
#SBATCH --partition={config.get('partition', 'all')}
#SBATCH --nodes={config.get('nodes', '1')}
#SBATCH --ntasks={config.get('ntasks', '24')}
#SBATCH --cpus-per-task=1
#SBATCH --mem={config.get('mem', '160G')}
#SBATCH --time={config.get('time', '1-01:00')}
#SBATCH --chdir={remote_job_dir}
#SBATCH --output=orca_output.out

jobname="{job_name}"

{env_block}

work_dir="{remote_job_dir}"
cd $work_dir

# orca run
$EBROOTORCA/bin/orca orca_input.inp > orca_output.out
"""

    def start_job(self, job_id, orca_path="orca"):
        """Launches ORCA 6 locally or via SLURM."""
        job_dir = self.get_job_dir(job_id)
        ...
        execution_mode = meta.get("execution_mode", "local")

        if execution_mode == "slurm":
            if not PARAMIKO_AVAILABLE:
                print("Paramiko not available for SLURM submission.")
                return False
            config = meta.get("slurm_config", {})
            host = config.get("host")
            user = config.get("user")
            key_path = config.get("key_path")
            remote_base = config.get("remote_dir", "").rstrip('/')
            
            if not all([host, user, key_path, remote_base]):
                return False
            
            remote_job_dir = f"{remote_base}/{job_id}"
            script_content = self._generate_slurm_script(job_id, remote_job_dir, config)
            script_file = os.path.join(job_dir, "submit.sh")
            ...
            try:
                ...
                sftp = ssh.open_sftp()
                ssh.exec_command(f"mkdir -p {remote_job_dir}")
                
                remote_inp_file = f"{remote_job_dir}/orca_input.inp"
                remote_script_file = f"{remote_job_dir}/submit.sh"
                
                sftp.put(inp_file, remote_inp_file)
                sftp.put(script_file, remote_script_file)
                sftp.close()
                
                cmd = f"cd {remote_job_dir} && dos2unix submit.sh && sbatch submit.sh"
                ...
                meta["slurm_job_id"] = slurm_id
                meta["status"] = "running"
                meta["started"] = datetime.now().isoformat()
                meta["last_poll_time"] = time.time()
                self._write_meta(job_dir, meta)
                return True
            except Exception as e:
                print(f"SLURM Start Error: {e}")
                meta["status"] = "failed"
                meta["error"] = str(e)
                self._write_meta(job_dir, meta)
                return False
    ...
    def kill_job(self, job_id):
        """Kills the background ORCA process or SLURM job."""
        job_dir = self.get_job_dir(job_id)
        meta_file = os.path.join(job_dir, "job_meta.json")
        if not os.path.exists(meta_file):
            return False
        ...
        if execution_mode == "slurm":
            ...
            if slurm_id and PARAMIKO_AVAILABLE:
                config = meta.get("slurm_config", {})
                try:
                    ssh = paramiko.SSHClient()
                    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                    ssh.connect(config.get("host"), username=config.get("user"), key_filename=config.get("key_path"))
                    ssh.exec_command(f"scancel {slurm_id}")
                    ssh.close()
                except:
                    pass
        else:
    ...
    def _update_job_status(self, job_id, meta):
        ...
        if execution_mode == "slurm":
            ...
            # Rate limit polling to every 15 seconds
            last_poll = meta.get("last_poll_time", 0)
            ...
            if PARAMIKO_AVAILABLE:
                ...
                try:
                    ...
                    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
                    ssh.connect(config.get("host"), username=config.get("user"), key_filename=config.get("key_path"))
                    stdin, stdout, stderr = ssh.exec_command(f"squeue -j {slurm_id} -h")
                    status_out = stdout.read().decode().strip()
                    
                    if status_out:
                        process_alive = True
                    else:
                        # Job is done, download files
                        remote_base = config.get("remote_dir", "").rstrip('/')
                        remote_job_dir = f"{remote_base}/{job_id}"
                        sftp = ssh.open_sftp()
                        try:
                            ...
                            for file in remote_files:
                                remote_file_path = f"{remote_job_dir}/{file}"
                                local_file_path = os.path.join(job_dir, file)
                                try:
                    ...
                    print(f"SLURM poll error: {e}")
                    process_alive = True # Keep alive if network issue
        else:
            pid = meta.get("pid")
            if pid:
                if sys.platform == "win32":
        ...
        if os.path.exists(out_file) and not process_alive:
            try:
