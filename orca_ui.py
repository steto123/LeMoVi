...
class OrcaSetupDialog(QDialog):
    ...
    def __init__(self, parent, xyz_content, job_manager, default_name="orca_calc"):
        ...
        self.basis_combo.addItems([
            ...
            # Pople Basis Sets
            "6-31G", "6-31G(d)", "6-31G(d,p)", "6-31+G(d)", "6-31+G(d,p)", "6-31++G(d,p)", "6-311G(d)", "6-311G(d,p)", "6-311+G(2d,p)",
        ...
        self.exec_group = QGroupBox("Execution Mode")
        self.exec_group.setStyleSheet("QGroupBox { border: 1px solid #444; border-radius: 4px; margin-top: 10px; } QGroupBox::title { subcontrol-origin: margin; left: 10px; padding: 0 3px 0 3px; color: #b0bec5; }")
        exec_layout = QFormLayout(self.exec_group)
        
        self.exec_mode_combo = QComboBox()
        self.exec_mode_combo.addItems(["local", "slurm"])
        self.exec_mode_combo.currentTextChanged.connect(self._update_slurm_visibility)
        exec_layout.addRow("Run on:", self.exec_mode_combo)
        
        self.slurm_widget = QWidget()
        slurm_form = QFormLayout(self.slurm_widget)
        slurm_form.setContentsMargins(0, 0, 0, 0)
        
        self.slurm_host = QLineEdit("login1.hpc.uni-potsdam.de")
        self.slurm_user = QLineEdit("sthomas")
        self.slurm_key = QLineEdit(r"C:\Users\wotan\.ssh\id_ed25519")
        self.slurm_remote_dir = QLineEdit("/work/sthomas/orca6")
        
        # Load saved settings from external JSON file
        import json, os
        self.slurm_settings_file = os.path.join(os.path.dirname(os.path.abspath(__file__)), "slurm_settings.json")
        self.slurm_defaults = {
            "ssh": {
                "host": "login1.hpc.uni-potsdam.de",
                "user": "sthomas",
                "key_path": r"C:\Users\wotan\.ssh\id_ed25519",
                "remote_dir": "/work/sthomas/orca6"
            },
            "job": {
                "partition": "all",
                "nodes": "1",
        ...
        if os.path.exists(self.slurm_settings_file):
            try:
                with open(self.slurm_settings_file, "r", encoding="utf-8") as f:
                    loaded = json.load(f)
                    for k in self.slurm_defaults:
                        if k in loaded:
                            if isinstance(self.slurm_defaults[k], dict):
                                self.slurm_defaults[k].update(loaded[k])
                            else:
                                self.slurm_defaults[k] = loaded[k]
            except Exception as e:
                print(f"Error loading slurm_settings.json: {e}")
        else:
            try:
                with open(self.slurm_settings_file, "w", encoding="utf-8") as f:
                    json.dump(self.slurm_defaults, f, indent=4)
            except Exception:
                pass
                
        self.slurm_host.setText(self.slurm_defaults["ssh"].get("host", ""))
        self.slurm_user.setText(self.slurm_defaults["ssh"].get("user", ""))
        self.slurm_key.setText(self.slurm_defaults["ssh"].get("key_path", ""))
        self.slurm_remote_dir.setText(self.slurm_defaults["ssh"].get("remote_dir", ""))
        
        slurm_form.addRow("SSH Host:", self.slurm_host)
        slurm_form.addRow("SSH User:", self.slurm_user)
        slurm_form.addRow("SSH Key:", self.slurm_key)
        slurm_form.addRow("Remote Dir:", self.slurm_remote_dir)
        
        # SLURM Job Settings
        self.slurm_part = QLineEdit(self.slurm_defaults["job"].get("partition", "all"))
        self.slurm_nodes = QLineEdit(self.slurm_defaults["job"].get("nodes", "1"))
        self.slurm_ntasks = QLineEdit(self.slurm_defaults["job"].get("ntasks", "24"))
        self.slurm_mem = QLineEdit(self.slurm_defaults["job"].get("mem", "160G"))
        self.slurm_time = QLineEdit(self.slurm_defaults["job"].get("time", "1-01:00"))
        
        slurm_form.addRow("Partition:", self.slurm_part)
        slurm_form.addRow("Nodes:", self.slurm_nodes)
        slurm_form.addRow("Tasks (Cores):", self.slurm_ntasks)
        slurm_form.addRow("Memory:", self.slurm_mem)
        slurm_form.addRow("Time Limit:", self.slurm_time)
        
        exec_layout.addRow("", self.slurm_widget)
        self.slurm_widget.setVisible(False)
    ...
    def _update_slurm_visibility(self, mode):
        self.slurm_widget.setVisible(mode == "slurm")
    ...
    def launch_job(self):
        ...
        if has_heavy and basis.startswith("6-31"):
            reply = QMessageBox.warning(
                ...
                "but you have selected a Pople basis set (6-31G / 6-311G). Pople basis sets are "
        ...
        if execution_mode == "slurm":
            slurm_config = {
                "host": self.slurm_host.text(),
                "user": self.slurm_user.text(),
                "key_path": self.slurm_key.text(),
                "remote_dir": self.slurm_remote_dir.text(),
                "partition": self.slurm_part.text(),
                "nodes": self.slurm_nodes.text(),
                "ntasks": self.slurm_ntasks.text(),
                "mem": self.slurm_mem.text(),
                "time": self.slurm_time.text(),
                "environment": self.slurm_defaults.get("environment", [])
            }
            # Save settings back to JSON
            self.slurm_defaults["ssh"]["host"] = slurm_config["host"]
            self.slurm_defaults["ssh"]["user"] = slurm_config["user"]
            self.slurm_defaults["ssh"]["key_path"] = slurm_config["key_path"]
            self.slurm_defaults["ssh"]["remote_dir"] = slurm_config["remote_dir"]
            
            self.slurm_defaults["job"]["partition"] = slurm_config["partition"]
            self.slurm_defaults["job"]["nodes"] = slurm_config["nodes"]
            self.slurm_defaults["job"]["ntasks"] = slurm_config["ntasks"]
            self.slurm_defaults["job"]["mem"] = slurm_config["mem"]
            self.slurm_defaults["job"]["time"] = slurm_config["time"]
...
class TMSReferenceDialog(QDialog):
    ...
    def __init__(self, workspace_dir=None, current_method=None, current_basis=None, parent=None,
        ...
        self.search_input.setPlaceholderText("e.g. B3LYP, HF, 6-31G(d), Chloroform, CPCM...")
